...
% t = t_2Sec.t_arr;
% c_air = C_air_vector_1;%/max(C_air_vector_1);
% t = t_arr;
% dt = mean(diff(t));
% Nt   = length(t);
...
%% Air Concentration at Receiver (from 'multiple_puff_ch_model_t_Rx.m')
param = load("ClassC_5d_5m_2.mat");
c_air = param.C_air_vector_1;
t = param.t_arr;
dt = mean(diff(t));
Nt = length(t);
Md = size(c_air,1);

% save("Rec_5u_5d_5m_2","t","C_L_ODE","C_L_SOL","C_L_SOL_2");
...
C_L_ODE = zeros(Md,Nt);
C_L_SOL = zeros(Md,Nt);
C_L_SOL_2 = zeros(Md,Nt);

%% Plant uptake ODE (implicit Euler)
for m = 1:Md
    for n = 1:Nt-1
        C_L_ODE(m,n+1) = ...
            ( C_L_ODE(m,n) + dt*beta*c_air(m,n+1) ) / ...
            ( 1 + dt*gamma );
    end
end
...
for m = 1:Md
    for n = 2:Nt
        ...
        C_L_SOL(m,n) = beta * trapz(t(1:n), kernel .* c_air(m,1:n));
    end
end

%% 
CL0 = c_air(2)*K_LA;                 % initial internal concentration
for m = 1:Md
    for n = 2:Nt
        kernel = exp(-gamma * (t(n) - t(1:n)));
        C_L_SOL_2(m,n) = exp(-gamma * t(n)) * CL0 ...
              + beta * trapz(t(1:n), kernel .* c_air(m,1:n));
    end
end
